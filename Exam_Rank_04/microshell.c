...
/*   Updated: 2023/11/25 15:22:22 by eavedill         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include <stdlib.h>
#include <stdio.h>
...
int exec_cmd(char **ac, char **env, int tmp_fd)
{
	...
	close(tmp_fd);
	execve(ac[0],ac,env);
	ft_putstr_err("error: cannot execute", ac[0]);
	return 0;
}
...
int main (int av, char **ac, char **env)
{
	...
	while (++i <= av)
	{
		...
		i = j;
		if(strcmp(comando[0], "cd") == 0 || strcmp(comando[0], "/usr/bin/cd") == 0)
		{
			my_cd(comando);
		}
		else if (!ac[i] || strcmp(ac[i], ";") == 0)
		{
			if (fork() == 0)
			{
