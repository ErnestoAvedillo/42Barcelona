...
{
	if (ft_strchr(str,PLUS_FLAG))
		return (PLUS_FLAG);
	if (ft_strchr(str,MINUS_FLAG))
		return (MINUS_FLAG);
	if (ft_strchr(str,POINT_FLAG))
		return (POINT_FLAG);
	if (ft_strchr(str,SPACE_FLAG))
		return (SPACE_FLAG);
	if (ft_strchr(str,POUND_FLAG))
		return (POUND_FLAG);
	return (NONE_FLAG);
}
...
t_form_data	*fill_list(char *str, int pos)
{
	...
	straux = ft_substr (str, pos, cur_pos - pos);
	if (!straux)
		format_def->longfield = -1;
	format_def->flag = find_flag (straux);
	if (format_def->flag == POINT_FLAG)
	{
		format_def->longfield = get_len_field(straux);
		format_def->iszero = get_len_zeros(straux);
	}
	return (format_def);
}
